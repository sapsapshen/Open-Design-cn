...
function printRootHelp() {
  ...
  od mcp [--daemon-url <url>]
      ...
      running Open Design - Chinese Optimized Version daemon. Wire it into a coding agent
      (Claude Code, Cursor, VS Code, Zed, Windsurf) in another repo
      to pull files from a local Open Design - Chinese Optimized Version project without
      exporting a zip.
...
Runs Tavily-backed shallow research through the local Open Design - Chinese Optimized Version daemon.
  ...
  if (code === 'EPERM' || code === 'ENETUNREACH') {
    console.error(
      'hint: outbound connect was denied by a sandbox. If you launched ' +
        ...
        'network policy. The Open Design - Chinese Optimized Version daemon itself is unaffected - it can be ' +
...
tool calls to a running Open Design - Chinese Optimized Version daemon. Wire it into a coding agent
in another repo so the agent can pull files from a local Open Design - Chinese Optimized Version
project without exporting a zip every iteration.

Options:
  --daemon-url <url>   Open Design - Chinese Optimized Version daemon HTTP base URL. Resolution
...
Tools exposed:
  list_projects                  list every Open Design - Chinese Optimized Version project
...
Open Design - Chinese Optimized Version; get_artifact and get_file additionally default to the
...
→ MCP server in the Open Design - Chinese Optimized Version app. Read-only by design; the daemon
