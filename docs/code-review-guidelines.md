...
Run this test **before** reviewing implementation details. A PR passes only when the changed behavior is visible in, required by, or directly validates an Open Design - Chinese Optimized Version–owned surface.

A PR passes the relevance test when **all** of the following hold:

- The PR description identifies the Open Design - Chinese Optimized Version feature, command, protocol, package, resource format, or runtime path being changed.
- Tests exercise existing Open Design - Chinese Optimized Version flows through their public seams: artifact generation/rendering via documented daemon APIs, daemon HTTP/SSE endpoints, web UI on shipped routes, desktop/packaged launch behavior, sidecar status/IPC, `tools-dev`/`tools-pack` lifecycle commands, skills/design-system/craft resource loading, or documented cross-boundary smoke behavior. Rendering domain content through a generic primitive does **not** count.
...
- New sample content is a maintained fixture for an Open Design - Chinese Optimized Version capability, minimal enough to support the test, and stored under the [first-party fixture rule](#first-party-fixture-rule).
...
Domain-specific UI or content (a customer vertical, marketing experiment, unrelated rendering demo, arbitrary product page) is out of scope unless it is explicitly a first-party Open Design - Chinese Optimized Version fixture and the PR explains which Open Design - Chinese Optimized Version capability it validates.
...
A generally reasonable direction (e.g. "add Arabic/RTL coverage") is **not** sufficient justification to keep an unsalvageable PR open. Close with a comment that names which condition was met and asks for a fresh PR that uses the actual Open Design - Chinese Optimized Version app and harness. If the contributor disputes the close, escalate to another maintainer rather than reopening unilaterally.
...
- Open Design - Chinese Optimized Version app/mode/source/stamp details hard-coded inside generic `packages/sidecar` or `packages/platform` code.
...
Treat scope as an approval gate. This repository is for Open Design - Chinese Optimized Version itself: the local web/daemon/desktop product, its sidecar and packaging infrastructure, shared contracts, development/release tools, e2e coverage of those surfaces, and first-party skills/design-system/craft resources consumed by that product.
...
- `packages/sidecar-proto` — Open Design - Chinese Optimized Version sidecar business protocol; app/mode/source constants, namespace validation, stamp fields/flags, IPC message schema, status shapes, error semantics.
...
- Adds tests, fixtures, scripts, or UI for a separate product/domain that does not exercise an existing Open Design - Chinese Optimized Version feature.
...
- The skill supports design work in Open Design - Chinese Optimized Version — artifact generation, visual design, layout, branding, design-system application, interaction design, accessibility review, design critique.
...
- Instructions are general-purpose enough for Open Design - Chinese Optimized Version workflows, not a single unrelated external service or business process.
...
- It adds broad external integrations or credentials not necessary for a design-focused Open Design - Chinese Optimized Version workflow.
...
- **Domain-specific app smuggled in as tests.** A PR adding tests for a "Quranic Arabic Learning App" with selectors like `#dashboard`, `#exercise`, `.vocabulary-item`, `.arabic-word` when those surfaces are not part of Open Design - Chinese Optimized Version — violates the "tests target real routes/DOM/APIs" rule.
- **Standalone-app E2E URLs.** Playwright tests hard-coding `http://localhost:17573/index.html` instead of using the configured Open Design - Chinese Optimized Version app flow — violates the "use the existing harness, base URL, and lifecycle conventions" rule.
...
A reasonable underlying intent (e.g. "we need RTL or Arabic coverage") does not justify keeping such a PR open. Ask for a fresh PR that exercises the actual Open Design - Chinese Optimized Version app through the existing harness.
